...
variable "comment" {
  ...
  default     = ""
  description = <<-EOT
    Пояснение к записи. К нему всегда добавляется managed-by:terraform —
    по этой пометке в зоне отличаются управляемые записи от заведённых
    руками. Именно ручные со временем превращаются в сирот, указывающих
